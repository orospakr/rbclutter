...
static VALUE
rbclt_behaviour_rotate_initialize (VALUE self, VALUE alpha, VALUE axis, VALUE direction,
				   VALUE angle_begin, VALUE angle_end)
{
  ClutterBehaviour *behaviour;

  behaviour = clutter_behaviour_rotate_new (RVAL2GOBJ (alpha),
...
{
  ClutterBehaviourRotate *brotate = CLUTTER_BEHAVIOUR_ROTATE (RVAL2GOBJ (self));
  ClutterVertex *vertex = (ClutterVertex *) RVAL2BOXED (center, CLUTTER_TYPE_VERTEX);
  clutter_behaviour_rotate_set_center (brotate,
				       CLUTTER_UNITS_TO_INT (vertex->x),
				       CLUTTER_UNITS_TO_INT (vertex->y),
				       CLUTTER_UNITS_TO_INT (vertex->z));
...
static VALUE
rbclt_behaviour_rotate_set_bounds (VALUE self, VALUE angle_begin, VALUE angle_end)
{
  ClutterBehaviourRotate *brotate = CLUTTER_BEHAVIOUR_ROTATE (RVAL2GOBJ (self));
  clutter_behaviour_rotate_set_bounds (brotate, NUM2DBL (angle_begin), NUM2DBL (angle_end));
  return self;
}
...
{
  VALUE klass = G_DEF_CLASS (CLUTTER_TYPE_BEHAVIOUR_ROTATE, "BehaviourRotate", rbclt_c_clutter);
